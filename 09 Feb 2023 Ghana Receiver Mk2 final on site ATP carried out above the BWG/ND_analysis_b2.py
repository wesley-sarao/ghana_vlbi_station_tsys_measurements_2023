...
plt.figure(1)
plt.clf()
plt.plot(freq/1e6+5982,teff, linewidth=1,color='r', label='$T_{e}$')
plt.plot(freq/1e6+5982,teff_5k, linewidth=1,color='r', label='$5k$')
plt.plot(freq/1e6+5982,teff_20k, linewidth=1,color='r', label='$20k$')
...
plt.plot([5982+freq[200]/1e6,5982+freq[600]/1e6],
         [np.mean(teff_5k[200:600]-teff[200:600]),
          np.mean(teff_5k[200:600]-teff[200:600])], 
          linewidth=3,
          color='#8F94CC',
          label='5k mean value over the band = %.2f k'%(np.mean(teff_5k[200:600]-teff[200:600])))

plt.fill_between([6550,6950,6950,6550],
                 [5.45,5.45,4.55,4.55],
                 facecolor='r',
                 alpha=0.1,
                 edgecolor='r',
...
#plt.plot([6550,6950],[5.45,5.45], linewidth=1,color='b', label='Specification limit = 5.45 k')
#plt.plot([6550,6950],[4.55,4.55], linewidth=1,color='b', label='Specification limit = 4.55 k')
#    plt.plot([6550,6950],[110,110], linewidth=1,color='r', label='Spec')
...
#plt.plot(5749-freq/1e6,teff_20k-teff, linewidth=1,color='r', label='$20k$')
plt.plot([5982+freq[200]/1e6,5982+freq[600]/1e6],
         [np.mean(teff_20k[200:600]-teff[200:600]),
          np.mean(teff_20k[200:600]-teff[200:600])], 
          linewidth=3,
          color='#8F94CC',
          label='20k mean value over the band = %.2f k'%(np.mean(teff_20k[200:600]-teff[200:600])))
...
#plt.plot([6550,6950],[22.5,22.5], linewidth=1,color='b', label='Specification limit = 22.5 k')
#plt.plot([6550,6950],[17.5,17.5], linewidth=1,color='b', label='Specification limit = 17.5 k')
#    plt.plot([6550,6950],[110,110], linewidth=1,color='r', label='Spec')
plt.title(band+" effective noise injection temperature")
plt.xlabel("Frequency (MHz)")
plt.ylabel("Noise Temperature (K)")
