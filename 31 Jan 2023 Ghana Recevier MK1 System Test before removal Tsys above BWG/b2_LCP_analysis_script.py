...
band = 'B2LCP'
meas = 'nom_gain_-5dB_BWG_mode_3' #'_20k_nd'
hot_load_location = 'above beam waveguide'
assumed_tcold = '10.7 K'
temp_hot_load_degrees_C = 31.4 # Read from the EMS 
sky_conditions = 'cloud cover 30-40%'
wind = 'low / no wind'
mode = 'mode 3'
gain = '-5 dB' #due to saturation of the ROACH


#-----------------------------------------------------------------------------
freq =np.load(band+'/DUTfreq.npy')
skyfreq = (3825 + 3445)*1e6 - freq
...
def plot_slope():
    ...
    plt.plot((3445-freq[data_s:data_f]/1e6)+3825, p(freq[data_s:data_f]/1e6),color='orange', alpha = 0.8,label='slope = %2.2f dB' %(400*(p.c[0])))
    
    plt.plot((3445-freq/1e6)+3825,
          (dutG_dB),
          linewidth=1,
          color='r')#label ='ripple = %2.2f dB'%(np.max(p(freq[data_s:data_f]/1e6)-(dutG_dB[data_s:data_f]))+np.max((dutG_dB[data_s:data_f])-p(freq[data_s:data_f]/1e6))))
...
def plot_ripple():
    ...
    plt.plot(freq[data_s:data_f]/1e6, p(freq[data_s:data_f]/1e6)-(dutG_dB[data_s:data_f]),color='orange', alpha = 0.8,label=' peak to peak gain ripple = %2.2f dB' %(np.max(p(freq[data_s:data_f]/1e6)-(dutG_dB[data_s:data_f]))+np.max((dutG_dB[data_s:data_f])-p(freq[data_s:data_f]/1e6))))
...
def plot_DUTTeff():
    ...
    plt.fill([6618,6618,6718,6718],[125,140,140,125],
             color='red',
             alpha=0.3,
             hatch="/",
             edgecolor='black',
    ...
    plt.fill([6550,6550,6950,6950],[0,140,140,0],
             color='g',
             alpha=0.05,
             label="6550 MHz to 6950 MHz")    
...
# Teff_B2_LCP = np.load("B2LCP/Teff_B2LCP_nom_gain_-5d_BWG_mode_3.npy")
# Teff_B2_RCP = np.load("B2RCP/Teff_B2RCP_nom_gain_-5d_BWG_mode_3.npy")
...
# plt.figure(5)
# plt.clf()
# plt.plot((3445-freq/1e6)+3825,Teff_B2_LCP, linewidth=1, label='$T_e$ B2_LCP')
# plt.plot((3445-freq/1e6)+3825,Teff_B2_RCP, linewidth=1, label='$T_e$ B2_RCP')

# plt.plot([(3445-freq[data_s]/1e6)+3825,(3445-freq[data_f]/1e6)+3825],[Teff_B2_LCP_mn,Teff_B2_LCP_mn], linewidth=1, label='$T_e$ B2_LCP mean = %0.2f k'%Teff_B2_LCP_mn)
# plt.plot([(3445-freq[data_s]/1e6)+3825,(3445-freq[data_f]/1e6)+3825],[Teff_B2_RCP_mn,Teff_B2_RCP_mn], linewidth=1, label='$T_e$ B2_RCP mean = %0.2f k'%Teff_B2_RCP_mn)
...
Teff_B2_LCP_FH = np.load("B2LCP//Teff_B2LCP_nom_gain_-5dB_FH_mode_3.npy")
Teff_B2_LCP_BWG = np.load("B2LCP//Teff_B2LCP_nom_gain_-5dB_BWG_mode_3.npy")


plt.figure(6)
plt.clf()
plt.plot(skyfreq/1e6,Teff_B2_LCP_FH, linewidth=1, label='$T_e$ B2_LCP above FH')
plt.plot(skyfreq/1e6,Teff_B2_LCP_BWG, linewidth=1, label='$T_e$ B2_LCP above BWG')

plt.title("Band 2 LCP difference between Tsys taken above BWG and FH")
plt.xlabel("Frequency (MHz)")
plt.ylabel("Noise Temperature (K)")
plt.grid(b='on',which='both',color='black', linestyle='-', linewidth=0.5)
plt.ylim((60,110))
...
plt.plot(skyfreq/1e6,Teff_B2_LCP_BWG-Teff_B2_LCP_FH, linewidth=1, label='$T_e$ B2_LCP above BWG')

plt.title("Band 2 LCP Tsys taken above BWG minus tsys taken above FH")
plt.xlabel("Frequency (MHz)")
plt.ylabel("Noise Temperature (K)")
plt.grid(b='on',which='both',color='black', linestyle='-', linewidth=0.5)
plt.ylim((-10,10))
...
fig = plt.gcf()
fig.set_size_inches(8, 6, forward=True)
fig.savefig(band+'/Teff_'+band+'_'+meas+ 'Beamwaveguide mins Feedhorn', dpi=200)